...
export class DataService {
  ...
  getCustomers(): Observable<ICustomer[]> {
    return this.http.get<ICustomer[]>(this.baseUrl + 'customers.json').pipe(
      ...
      //Pipe. Like a hose that water flows through. As water flows through you can plug
      // in different types of filters (operators).
    );
  }
  ...
  getOrders(id: number): Observable<IOrder[]> {
    return this.http.get<IOrder[]>(this.baseUrl + 'orders.json').pipe(
      map((orders) => {
        ...
        // This is actually what the caller would get - custOrders;
        return custOrders;
      }),
      catchError(this.handleError)
    );
